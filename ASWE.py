...
def discriminator(input):
    with tf.variable_scope("D", reuse=tf.AUTO_REUSE):
        hidden = tf.layers.dense(input, EMBED_SIZE // 2, activation=tf.nn.tanh)
        hidden = tf.layers.dense(hidden, EMBED_SIZE // 4, activation=tf.nn.tanh)
...
def train():
    ...
    d_real = discriminator(label_embed)
    d_gen = discriminator(gen_embed)

    d_loss = tf.reduce_mean(d_gen) - tf.reduce_mean(d_real) - 10 * tf.reduce_mean(
        cosine_similarity(gen_embed, label_embed))

    g_loss = -tf.reduce_mean(d_gen) - 10 * tf.reduce_mean(cosine_similarity(gen_embed, label_embed))

    beta = tf.random_uniform(shape=[2 * BATCH_SIZE, 1], minval=0., maxval=1.)
    differences = gen_embed - label_embed
    interpolates = label_embed + (beta * differences)
    ...
    d_optimizer = tf.train.AdamOptimizer(learning_rate=1e-4,
                                         beta1=0.5,
    ...
    gen_similarity = cosine_similarity(gen_embed, label_embed)
    ...
    with tf.Session() as session:
        ...
        while (num_neg_loops < 2 * NUM_ITERATIONS) or (num_pos_loops < 2 * NUM_ITERATIONS):
            ...
            if (num_neg_loops < 2 * CONTEXT_ITERATIONS) and (num_pos_loops < 2 * CONTEXT_ITERATIONS):
                opt_j, loss_j_val = session.run([joint_optimizer, joint_loss], feed_dict=feed_dict)
                average_loss_j += loss_j_val

            if (num_neg_loops >= 2 * CONTEXT_ITERATIONS) and (num_pos_loops >= 2 * CONTEXT_ITERATIONS):
                opt_g, opt_d, loss_g_val, loss_d_val, vector_similarity = session.run(
                    [g_optimizer, d_optimizer, g_loss, d_loss, gen_similarity],
                    feed_dict=feed_dict)
                average_loss_g += loss_g_val
                average_loss_d += loss_d_val
                average_cosine = np.mean(vector_similarity)
                record_cosine.append(average_cosine)
            ...
            if step % 2000 == 0:
                ...
                if (num_neg_loops >= 2 * CONTEXT_ITERATIONS) and (num_pos_loops >= 2 * CONTEXT_ITERATIONS):
                    print("generator loss: {:.4f}, discriminator loss: {:.4f}".format(average_loss_g, average_loss_d))
                    np.set_printoptions(precision=2)
                    print(vector_similarity[:20])
                ...
                record_loss_g.append(average_loss_g)
                record_loss_d.append(average_loss_d)
...
if __name__ == '__main__':
    ...
    with open('vector/ASWE.pickle', 'wb') as handle:
